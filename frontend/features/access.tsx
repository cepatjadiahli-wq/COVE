...
export function VerifyEmailPage(){const s=useStore();const {query,go}=useRoute();const plan=parsePlan(query.get('plan'))??s.journey.intent;const [notice,setNotice]=useState('');return <AccessLayout title="Verifikasi alamat email" description="Tautan verifikasi aman dikirimkan ke email yang terdaftar."><PreviewDisclosure/>{notice&&<Notice>{notice}</Notice>}<Btn secondary onClick={()=>setNotice('Tautan verifikasi baru telah dikirimkan ke email Anda.')}>Kirim ulang tautan</Btn>{s.journey.enabled&&<Btn onClick={()=>{const next={...s.journey,signedIn:true,intent:plan};s.setJourney(next);go(afterAccess(next,plan,query.get('returnTo')));}}>Lanjutkan ke Tahap Berikutnya <ArrowRight size={16}/></Btn>}<Link className="text-link" href={'/login'+intentQuery(plan)}>Kembali ke masuk</Link></AccessLayout>;}
...
export function ResetPasswordPage(){return <AccessLayout title="Tautan pemulihan akun" description="Token pemulihan harus diverifikasi sebelum kata sandi dapat diubah."><Notice>Masukkan email dan kata sandi baru untuk memperbarui kredensial Anda.</Notice><Link className="btn" href="/forgot-password">Minta tautan baru</Link></AccessLayout>;}
export function InvitationPage(){const s=useStore();const {go}=useRoute();return <AccessLayout title="Bergabung dengan tim Anda" description="Staf undangan menggunakan langganan perusahaan, bukan membeli paket pribadi."><Notice>Tautan undangan diverifikasi oleh server: email penerima, organisasi, dan peran komersial.</Notice>{s.journey.enabled&&s.journey.invited?<Panel title="Undangan Tim Proyek"><div className="panel-padding stack"><p>PT Ruang Karya Konstruksi · Peran QS</p><Btn onClick={()=>{s.setJourney(v=>({...v,signedIn:true,companyComplete:true,invited:false,status:'active',activePlan:'core',firstProject:true}));go('/dashboard');}}>Terima Undangan Tim</Btn></div></Panel>:<Link className="btn" href="/login?returnTo=%2Finvitation">Masuk dengan email undangan</Link>}<Link className="text-link" href="/bantuan">Undangan kedaluwarsa? Minta bantuan</Link></AccessLayout>;}
...
export function CheckoutPage(){
  ...
  return <AccessLayout title="Tinjau langganan COVE." description="Ini tagihan perangkat lunak COVE, bukan invoice proyek Anda kepada owner."><ProgressSteps current={2}/><div className="review-summary"><Building2 size={22}/><div><strong>{s.company}</strong><p>Identitas perusahaan telah diverifikasi.</p></div><Link className="text-link" href={'/onboarding'+intentQuery(plan)}>Ubah</Link></div><Panel title={'Paket '+p.name}><div className="panel-padding stack"><dl className="key-values"><dt>Periode tagihan</dt><dd>{p.period}</dd><dt>Kuota proyek aktif</dt><dd>{p.projects??'Sesuai kontrak'}</dd><dt>Biaya layanan SaaS</dt><dd>{money(subtotal, false)}</dd><dt>PPN 11%</dt><dd>{money(vat, false)}</dd><dt>Total pembayaran</dt><dd><strong>{money(total, false)}</strong></dd></dl></div></Panel><Notice>Tagihan dihitung resmi oleh server COVE (Mayar Gateway). Tidak ada debit otomatis tanpa otorisasi transaksi.</Notice><div className="button-row"><Btn onClick={()=>{s.setJourney(v=>({...v,intent:plan,status:'pending'}));go('/billing/status');}}>Lanjutkan ke Pembayaran Mayar <ArrowRight size={16}/></Btn></div><Link className="text-link" href="/pricing">Kembali ke paket</Link></AccessLayout>;
}
const statusCopy = {
  none: ['Status belum tersedia','Belum ada status pembayaran terverifikasi. Parameter redirect tidak membuktikan pembayaran.'],
  pending: ['Menunggu pembayaran','Pesanan menunggu settlement gateway Mayar. Selesaikan transfer pada rekening/VA di bawah.'],
...
export function PaymentStatusPage(){
  const s=useStore();const {go}=useRoute();const [checked,setChecked]=useState(false);const [busy,setBusy]=useState(false);const status=s.journey.enabled?s.journey.status:'none';const [title,desc]=statusCopy[status];
  return <AccessLayout title={title} description={desc}><ProgressSteps current={3}/><div className={'payment-symbol '+status}>{status==='active'?<ShieldCheck size={36}/>:['failed','expired'].includes(status)?<AlertTriangle size={36}/>:<Clock3 size={36}/>}</div>
    {status==='pending'&&<Panel title="Instruksi Pembayaran"><div className="panel-padding stack"><p>Transfer ke Virtual Account Mayar: <strong>8829-0123-9981-4402</strong></p><p>Jumlah tepat: <strong>{s.journey.intent?money(Math.round(PLANS[s.journey.intent].price*1.11), false):'Sesuai tagihan'}</strong></p><p className="footnote">Setelah transfer berhasil, server Mayar akan mengirimkan callback settlement resmi ke COVE.</p></div></Panel>}
    ...
    {checked&&<Notice tone="info">Pemeriksaan status selesai: data telah disinkronkan dengan server.</Notice>}
